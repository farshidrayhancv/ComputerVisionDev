...
if __name__ == "__main__":
    ...
    # Remove the +1 from line 340
    # Assume batch size = 2, num_queries = 100, num_classes = 80
    outputs = {
        'pred_logits': Tensor(np.random.rand(2, 100, 80).astype(np.float32)),
        'pred_boxes': Tensor(np.random.rand(2, 100, 4).astype(np.float32)),
    }

    # Assume batch size = 2, num_target_boxes_0 = 10
    targets = [
        {
            'labels': Tensor(np.random.randint(0, 80, 10).astype(np.int32)),
            'boxes': Tensor(np.random.rand(10, 4).astype(np.float32)),
        }
    ]

    # Instantiate SetCriterion and call construct
    criterion = SetCriterion(num_classes=79, matcher=build_matcher(), weight_dict={}, eos_coef=0.1, losses=['labels', 'boxes', 'cardinality'])
    losses = criterion.construct(outputs, targets)

    # Print the total loss and individual losses
    total_loss = sum(losses.values()).asnumpy()
