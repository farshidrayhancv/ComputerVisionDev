...
class MultiHeadAttention(nn.Cell):
    ...
    def construct(self, query, key, value, mask=None):
        ...
        if mask is not None:
            ...
            attention = ops.MaskedFill()(attention, mask == 1, Tensor(-np.inf*1.0))

        softmax = ops.Softmax(-1)
        attention = self.dropout(softmax(attention))
        output = batmatmul(attention, v_w)
        output = transpose(output, (0, 2, 1, 3))
        output = output.view(bs, -1, self.n_heads * (self.hid_dim // self.n_heads))
